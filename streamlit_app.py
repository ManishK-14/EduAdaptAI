...
from modules.file_processor import process_uploaded_file, process_text_input, save_uploaded_file, MAX_FILE_SIZE_MB
from modules.gemini_service import process_lesson_content, process_lesson_from_pdf, process_lesson_from_video
from modules.lesson_parser import parse_gemini_output, LessonData
from modules.adaptation_engine import render_adapted_content, ACCESSIBILITY_MODES
from modules.quiz_engine import render_quiz
from modules.progress_tracker import save_quiz_result, get_progress_summary
from utils.audio_service import render_audio_player, render_browser_tts_button
from utils.visual_components import render_visual_summary_cards
# ─────────────────────────────────────────────
...
def inject_custom_css():
    ...
    <style>
        ...
        /* Sidebar styling — dark text on light background (readability) */
...
def render_input_section(input_mode: str) -> tuple[str | None, Path | None, Path | None]:
    ...
    - PDF/Text modes match the original app; Video mode sets video_path only.
    """
    ...
    if "Upload PDF" in input_mode:
        ...
        video_file = st.file_uploader(
            ...
            type=["mp4", "mov"],
            key="video_uploader",
            help="Gemini will analyze visuals and narration.",
        )
        if video_file:
            try:
                ...
                st.success(f"✅ Video saved: **{video_file.name}** — ready to process.")
...
def render_processing_section(
    ...
    - extracted_text set → text lesson (including text extracted from PDF)
    - else video_path set → video File API
    - else pdf_path set → scanned PDF multimodal
    """
        ...
        with st.status("🧠 Processing lesson with Gemini AI...", expanded=True) as status:
            try:
                if extracted_text is not None:
                    ...
                    st.write("🎥 Waiting for file to become ACTIVE (may take a minute)...")
                    st.write("👁️ Gemini is analyzing the video...")
...
def render_lesson_output(lesson: LessonData):
    ...
    with tab_audio:
        ...
        ">
            ...
            <span style="font-size: 1.1rem; font-weight: 600; margin-left: 0.5rem;">Audio Lesson</span>
            <p style="margin: 0.3rem 0 0 0; opacity: 0.9; font-size: 0.9rem;">Listen to the lesson narrated by AI voice</p>
        </div>
        """, unsafe_allow_html=True)

        # Two audio options in columns
        col_gtts, col_browser = st.columns(2)
        with col_gtts:
            st.markdown("**🤖 AI Voice (gTTS)**")
            st.caption("High-quality Google voice — generates MP3")
            render_audio_player(lesson.audio_script, label="Full Lesson Audio")
        with col_browser:
            st.markdown("**🌐 Instant Browser Voice**")
            st.caption("Plays instantly using your browser — no download")
            render_browser_tts_button(lesson.audio_script, "▶️ Play Lesson Now")

        # Show the script text in an expander
        with st.expander("📝 View Audio Script Text", expanded=False):
            st.markdown(
                f"""<div style="
                    font-size: 1.05rem;
                    line-height: 1.85;
    ...
    with tab_visual:
        render_visual_summary_cards(lesson.visual_summary)

    with tab_frames:
        if lesson.visual_frame_captions:
            st.caption("Chronological scene descriptions with timestamps (video lessons).")
            ...
            st.info(
                "No frame captions here — this section is filled when you process a **video** lesson."
            )
    ...
    if lesson and st.session_state.get("processing_complete"):
        ...
        ">
            ...
            <p style="color: #666; font-size: 1.05rem; max-width: 500px; margin: 0 auto;">
                ...
                <strong>Process with Gemini AI</strong> to generate accessible learning materials.
            </p>
        </div>
        """, unsafe_allow_html=True)
